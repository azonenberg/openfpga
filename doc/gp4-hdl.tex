...
(GP\_COUNT8 and GP\_COUNT14) in the device. Not all hard macro capabilities (such as reset) are currently supported for 
inference; if these capabilities are required then use explicit primitive instantiation. Future software releases will 
expand the set of counter features which may be inferred.
...
\item Not have a runtime reset
\item Be initialized to the same value by both underflow and by power-on reset
\item Not have any logic use the internal counter value. Only the ``underflow" signal may be used by surrounding logic.
\end{itemize}

\subsubsection{Counter Related Constraints}

As of now, there is no way to disable counter inference for a single register. The ``greenpak4\_counters" pass will 
attempt to infer a counter macro for every counter matching the requirements. A future toolchain version will likely 
include a ``no counter extraction" constraint which may be applied to the count register if fabric logic implementation 
is desired.

\clearpage
\subsubsection{Verilog Usage Example}

The example in Figure \ref{gp-countinfer-example} shows an example of how to infer a non-resettable down counter.
...
//Toggle the LED every time the counter underflows
output reg led = 0;
always @(posedge clk) begin
	if(count == 0)
		led <= ~led;
