...
results of attempting to constrain a 3-input combinatorial expression to a LUT2 site.
...
\begin{figure}[h]
\begin{lstlisting}[language=c]
ERROR: Multiple conflicting LOC constraints (COUNT8_2, COUNT8_4) are
...
\subsubsection{Constraint Values}
\begin{itemize}
\item \whenstyle{Flipflop (DFF cell or inferred 1-bit register)}\\
\strvaluestyle{DFF\_n} where \tokenstyle{n} is the flipflop number. Example: \strvaluestyle{DFF\_5}
\item \whenstyle{IOB (top-level module port OR IOB primitive)}\\
...
versa); for example a 2-input function may be constrained to a LUT3 or LUT4 site.
\item \whenstyle{Shift register}\\
...
\item \whenstyle{Voltage reference}\\
\strvaluestyle{VREF\_n} where \tokenstyle{n} is the number of the attached comparator. Example: \strvaluestyle{VREF\_2}
\item \whenstyle{Other} \\
While this constraint is legal to use on primitives that only have one instance in the device, this is redundant so the 
names of these sites are not documented.
\end{itemize}
