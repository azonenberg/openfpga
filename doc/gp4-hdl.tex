...
A simple synthesis script for \namestyle{Yosys} is shown in figure \ref{yscript}. The script synthesizes a single Verilog source file \texttt{Blinky.v}, with a top-level module \texttt{BlinkyTop}, to the netlist \texttt{Blinky.json} and
...
\begin{itemize}
\item \whenstyle{Non-scalar top-level module ports}\\All ports on top-level modules must be one bit wide. Vector nets
...
% ERROR: Cell $abc$104$auto$blifparse.cc:347:parse_blif$106 has invalid LOC constraint LUT2_0 (site is of type GP_2LUT, instance is of type GP_3LUT)

%ERROR: Multiple conflicting LOC constraints (COUNT8_2, COUNT8_4) are attached to cell "lfosc_cnt".
...
entity to be constrained. Note that complex combinatorial logic where multiple levels of LUTs are inferred cannot be 
constrained using this approach as the constraint only applies to the last LUT in the tree; such logic must be broken 
up in order to constrain the other LUTs.

\item Instantiated primitives may be constrained by placing a \tokenstyle{LOC} constraint on the primitive declaration,
or on any net driven by the primitive.

\end{itemize}

\subsubsection{Constraint Values}
\begin{itemize}
\item \whenstyle{Top-level module port}\\
\strvaluestyle{Pn} where \tokenstyle{n} is the pin number of the device. Example: \strvaluestyle{P3}, \strvaluestyle{P17}.
\item \whenstyle{Inferred 1-bit register}\\
...
to lock LUTs and hard IP to specific locations.
\end{itemize}
...
\pagebreak
\subsection{\tokenstyle{GP\_DAC}: Digital to Analog Converter (NOT IMPLEMENTED)}
\label{gp-dac}
